...
const StoryDetailPage = () => {
          ...
          for (const list of data || []) {
            try {
              ...
              if (storiesResponse.ok) {
                ...
                if (stories.some(story => story.id === parseInt(id))) {
                  listsWithStory.add(list.id);
                }
              }
            } catch (err) { console.error(`Error checking list ${list.id}:`, err); }
          }
  ...
  useEffect(() => {
    const fetchStoryDetails = async () => {
      try {
        ...
        } catch (err) { console.log('Not logged in or error checking follow status'); }
      } catch (err) {
    ...
    const handleStoryDataUpdated = (event) => {
      if (event.detail?.storyId === parseInt(id)) fetchStoryDetails();
    };
    window.addEventListener('storyDataUpdated', handleStoryDataUpdated);
    const handleVisibilityChange = () => { if (!document.hidden) fetchStoryDetails(); };
  ...
  const handleFollowToggle = async () => {
    try {
      if (following) { await unfollowStory(id); setFollowing(false); }
      else { await followStory(id); setFollowing(true); }
    } catch (err) {
  ...
  const handleCreateList = async () => {
    if (!newListName.trim()) { alert('Please enter a list name'); return; }
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch('http://localhost:4000/reading-lists', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ name: newListName, description: newListDescription, is_public: newListPrivacy })
      });
      if (response.ok) {
        const newList = await response.json();
        setReadingLists([...readingLists, newList]);
        setShowCreateListModal(false);
        setNewListName(''); setNewListDescription(''); setNewListPrivacy(true);
      } else { alert('Failed to create reading list'); }
    } catch (err) { console.error('Error creating reading list:', err); alert('Unable to create reading list'); }
  };
  ...
  const handleAddToList = async (listId) => {
    try {
      ...
      const response = await fetch(`http://localhost:4000/reading-lists/${listId}/stories`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ story_id: parseInt(id) })
      });
      if (response.ok) {
        ...
        alert(error.errorCode === 'ALREADY_ADDED' ? 'Story is already in this list' : 'Failed to add story to list');
      }
    } catch (err) { console.error('Error adding story to list:', err); alert('Unable to add story to list'); }
  };
  ...
  if (error || !story) {
    return (
      <Container className="mt-5">
        <Alert variant="warning">{error || "Story not found"}</Alert>
      </Container>
    );
  }

  // Shared gold button inline style
  const goldBtn = { backgroundColor: '#c9a84c', borderColor: '#c9a84c', color: '#1a1a1a', fontWeight: 600 };
  const goldOutlineBtn = { border: '2px solid #c9a84c', color: '#c9a84c', backgroundColor: 'transparent', fontWeight: 600 };
  const goldPlusBtn = {
  ...
  return (
    <Container className="mt-4">
      <Row>
                ...
                <div className="flex-grow-1">
                  ...
                  <div className="mb-3">
                    {story?.tags && story?.tags.map((tag) => (
                      <Badge key={tag?.id} style={{ backgroundColor: '#c9a84c', color: '#1a1a1a' }} className="me-1">
                        {tag?.name}
                      </Badge>
                    ))}
                  </div>
                  ...
                  <div className="d-flex gap-3 mb-3 text-muted">
                    ...
                    <span><i className="bi bi-eye me-1"></i>{story.read_count || 0} reads</span>
                    <span><i className="bi bi-heart me-1"></i>{story.vote_count || 0} votes</span>
                  </div>
                  ...
                  <div className="d-flex gap-2">
                    ...
                    <Button
                      style={following ? { backgroundColor: '#4a7c59', borderColor: '#4a7c59', color: 'white' } : goldOutlineBtn}
                      onClick={handleFollowToggle}
                    >
                      <i className={`bi ${following ? "bi-check" : "bi-plus"} me-1`}></i>
                      {following ? "Following" : "Follow"}
                    </Button>
                    <Button style={goldOutlineBtn} onClick={() => setShowLibraryModal(true)}>
                  ...
                  >
                    <div>
                      ...
                      {!chapter.is_published && (
                        <Badge style={{ backgroundColor: '#c9a84c', color: '#1a1a1a' }} className="ms-2">Draft</Badge>
                      )}
                    </div>
          ...
          <Card className="mt-4">
            ...
            <Card.Body>
              {comments.length > 0 ? (
                            ...
                            <div>
                              ...
                              {comment.chapter_title && (
                                <span className="text-muted ms-2">
                                  commented on{" "}
                                  <Link to={`/read/${comment.chapter_id}`} className="text-decoration-none" style={{ color: '#c9a84c' }}>
                ...
                <p className="text-center text-muted mb-0">No comments yet. Be the first to comment!</p>
              )}
            </Card.Body>
          </Card>
        </Col>
        ...
        <Col lg={4}>
          <Card className="mb-4">
            <Card.Header><h5 className="mb-0">About the Author</h5></Card.Header>
            <Card.Body>
              <div className="d-flex align-items-center mb-3">
                <img
                  ...
                  className="rounded-circle me-3"
                  style={{ width: "50px", height: "50px", border: '2px solid #c9a84c' }}
                />
                <div>
          ...
          <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div className="modal-content">
              ...
              <div className="modal-body">
                ...
                <div className="list-group list-group-flush" style={{ maxHeight: '400px', overflowY: readingLists.length > 5 ? 'auto' : 'visible' }}>
                  {readingLists.length === 0 ? (
                    ...
                    readingLists.filter(list => list.name.toLowerCase().includes(searchQuery.toLowerCase()))
                      .map((list) => {
                        const isAdded = listsContainingStory.has(list.id);
                        return (
                          <div key={list.id} className="list-group-item d-flex justify-content-between align-items-center py-3">
                            <div className="d-flex align-items-center gap-3">
                              <div className="d-flex align-items-center justify-content-center rounded"
                                style={{ width: '40px', height: '60px', backgroundColor: '#e8e0d0' }}>
                                <i className="bi bi-book" style={{ color: '#c9a84c' }}></i>
                              </div>
                              <div>
              ...
              <div className="modal-footer border-0 bg-light">
                <button type="button" className="btn btn-link text-decoration-none"
                  style={{ color: '#c9a84c' }}
                  onClick={() => { setShowLibraryModal(false); setShowCreateListModal(true); }}>
            ...
            <div className="modal-content">
              <div className="modal-header border-0">
                ...
                <button type="button" className="btn-close"
                  onClick={() => { setShowCreateListModal(false); setNewListName(''); setNewListDescription(''); setNewListPrivacy(true); }}>
                </button>
              </div>
              <div className="modal-body">
                <div className="mb-3">
                  <label className="form-label fw-semibold">List Name</label>
                  <input type="text" className="form-control" placeholder="My Reading List"
                    value={newListName} onChange={(e) => setNewListName(e.target.value)} />
                </div>
                <div className="mb-3">
                  <label className="form-label fw-semibold">Description (optional)</label>
                  <textarea className="form-control" rows="3" placeholder="What's this list about?"
                    value={newListDescription} onChange={(e) => setNewListDescription(e.target.value)} />
                </div>
                <div className="mb-3">
              ...
              <div className="modal-footer border-0">
                ...
                <button type="button" className="btn rounded-pill px-4"
                  style={goldBtn} onClick={handleCreateList}>
                  Create List
                </button>
              </div>
            </div>
